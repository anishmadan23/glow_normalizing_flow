...
parser.add_argument('--interpolate', action='store_true', help='Interpolate between imgs for gifs')
parser.add_argument('--interpolate_report', action='store_true', help='Interpolate between 2 imgs')

parser.add_argument('--manifold_viz', action='store_true', help='Viz the manifold')
...
parser.add_argument('--x1', type=str, help='Path to image file to interpolate')
parser.add_argument('--x2', type=str, help='Path to image file to interpolate')
...
def data_dependent_init(model, args):
    ...
    dataloader = fetch_dataloader(args, train=True, data_dependent_init=False)            # data dependent init changed to False
...
@torch.no_grad()
def interpolate(model, args, dataloader):
    model.eval()
    base_path = '/media/tiwari/My Passport/lokender/anish/normalizing_flows/datasets/'
    starting_imgs = ['000053.jpg','000129.jpg','000174.jpg','000272.jpg','002611.jpg','002797.jpg','000214.jpg','002803.jpg','000355.jpg']
    target_img = '002844.jpg'
    zs = []
    dataset = dataloader.dataset
    # x1_name = args.x1.split('/')[-1].split('.')[0]
    # x2_name = args.x2.split('/')[-1].split('.')[0]

    # img1 = dataset.transform(Image.open(args.x1).convert('RGB'))
    # img2 = dataset.transform(Image.open(args.x2).convert('RGB'))
    # img1 = img1.unsqueeze(0).to(args.device)
    ...
    # for alpha_z in alphas:
    #     new_z = []
    #     for i in range(len(z1)):
    #         combined_z = alpha_z*z1[i]+(1-alpha_z)*z2[i]
    #         new_z.append(combined_z)
    #     x_hat,_ = model.inverse(new_z,z_std=z_std)
    #     xs.append(x_hat.squeeze_(0))

    target_img = dataset.transform(Image.open(base_path+target_img).convert('RGB'))
    target_img = target_img.unsqueeze(0).to(args.device)
    target_z,_ = model(target_img)
    for alpha_z in alphas:
        print('Running for alpha',alpha_z)
        xs = []
        for img in starting_imgs:
            new_z = []

            start_img = dataset.transform(Image.open(base_path+img).convert('RGB'))
            start_img = start_img.unsqueeze(0).to(args.device)
            start_z,_ = model(start_img)

            for i in range(len(start_z)):
                combined_z = (1-alpha_z)*start_z[i]+alpha_z*target_z[i]
                new_z.append(combined_z)
            x_hat,_ = model.inverse(new_z,z_std=z_std)
            
            xs.append(x_hat.squeeze_(0))
        save_name = 'common_target_alpha_'+str(alpha_z)+'_z_std_'+str(z_std)+'_.png'
        save_image(xs,os.path.join(args.output_dir, save_name), nrow=3)
    

    # print(x1_hat.shape,x_hat.shape)

    # save_name = x1_name+'_'+x2_name+'_z_std_'+str(z_std)+'_.png'
    # # save_list = [x1_hat,x_hat]
    # save_image(xs, os.path.join(args.output_dir, save_name), nrow=len(alphas))

@torch.no_grad()
def interpolate_report(model, args, dataloader):
    model.eval()
    base_path = '/media/tiwari/My Passport/lokender/anish/normalizing_flows/datasets/'
    # starting_imgs = ['000053.jpg','000129.jpg','000174.jpg','000272.jpg','002611.jpg','002797.jpg','000214.jpg','002803.jpg','000355.jpg']
    # target_img = '002844.jpg'
    ...
    save_name = x1_name+'_'+x2_name+'_rep_z_std_'+str(z_std)+'_.png'
    # save_list = [x1_hat,x_hat]
    save_image(xs, os.path.join(args.output_dir, save_name), nrow=len(alphas))
...
@torch.no_grad()
def manifold_viz(model,args,dataloader):
    model.eval()
    phis = [-1.75,-1.5,-1.25,-1,-0.5,-0.25,0]
    phi_hats = [-1.75,-1.5,-1.25,-1]
    # phi_hats = [-1.75,-1.5,-1.25,-1]#, -0.5, -0.25,0,0.25,0.5,0.75]
    # phis = phi_hats.copy()


    phi_hats = [torch.Tensor([x]).to(args.device) for x in phi_hats]
    phis = [torch.Tensor([x]).to(args.device) for x in phis]

    # phi_hats = [torch.Tensor([1.0]).to(args.device)]#, -0.5, 3.4, -2.1, 0.0, -6.5]
    # phis = [torch.Tensor([3.4]).to(args.device)]#, -6.5]
    base_path = '/media/tiwari/My Passport/lokender/anish/normalizing_flows/datasets/manifold/'
    imgs = ['000050.jpg','000061.jpg','000063.jpg','000081.jpg']
    zs = []
    z_std=0.8
    dataset = dataloader.dataset
    z_size = None
    for img in imgs:
        img = dataset.transform(Image.open(base_path+img).convert('RGB'))
    ...
    xs = []
    for phi in phis:
        for phi_hat in phi_hats:
            print(phi,phi_hat)
            coeffs = [torch.cos(phi)*torch.cos(phi_hat),torch.cos(phi)*torch.sin(phi_hat),torch.sin(phi)*torch.cos(phi_hat),torch.sin(phi)*torch.sin(phi_hat)]
            new_z =None
            for idx,z in enumerate(zs):
                # print(z[0].shape,z[1].shape,z[2].shape,z[3].shape)
                if new_z is None:
                    my_z = [coeffs[idx]*z[i] for i in range(len(z))]
                    # new_z=my_z

                else:
                    # print(new_z[0].shape,new_z[1].shape,new_z[2].shape,new_z[3].shape)

                    my_z = [new_z[i]+coeffs[idx]*z[i] for i in range(len(z))]
                new_z=my_z
            
            x_hat,_ = model.inverse(new_z,z_std=z_std) 
            x_hat.squeeze_(0)
            xs.append(x_hat)    
                # print('Shape',my_z.shape)
                
    # print(new_z[0].shape,new_z[1].shape,new_z[2].shape,new_z[3].shape)

    save_name = 'manifold_rep'+'_z_std_'+str(z_std)+'_.png'
    save_image(xs,os.path.join(args.output_dir, save_name), nrow=len(phis))
...
if __name__ == '__main__':
    ...
    if args.interpolate:
        interpolate(model,args,test_dataloader)

    if args.interpolate_report:
        interpolate_report(model,args,test_dataloader)

    if args.manifold_viz:
        manifold_viz(model,args,test_dataloader)
